/**
 * Navbar Component for Version 1.0.0
 */
...
class Navbar {
  ...
  generateVersionList() {
    ...
    sortedVersions.forEach(version => {
      ...
      item.addEventListener('click', (e) => {
        ...
        if (isCurrent) {
          window.location.href = '/versions/1.0.0/';
        } else {
  ...
  initActiveLink() {
    ...
    links.forEach(link => {
      if (link.getAttribute('href') === currentPath || (currentPath === '/versions/1.0.0/' && link.getAttribute('href') === '/versions/1.0.0/')) {
